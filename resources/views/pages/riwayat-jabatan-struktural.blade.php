...
<div class="pagetitle">
    ...
    <nav>
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item">Riwayat Jabatan Struktural</li>
      </ol>
    </nav>
  </div><!-- End Page Title -->
  ...
  <div class="card-body">
      ...
      <h6 class="card-subtitle mb-2 text-body-secondary">Riwayat Jabatan Struktural</h6>
            ...
            @foreach ($data as $index => $item)
                <tr>
                    ...
                    <td>
                        ...
                        <form id="delete-form-{{ $item->id }}" action="{{ route('riwayat-jabatan-struktural.destroy', $item->id) }}" method="POST" style="display: none;">
                            @csrf
                            @method('DELETE')
                        </form>

                        <!-- Edit Modal -->
                        <div class="modal fade" id="editModalRiwayatJabatanStruktural{{ $item->id }}" tabindex="-1" aria-labelledby="editModalLabel{{ $item->id }}" aria-hidden="true">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="editModalLabel{{ $item->id }}">Edit Riwayat Jabatan Struktural</h5>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                    </div>
                                    <div class="modal-body">
                                        <form action="{{ route('riwayat-jabatan-struktural.update', $item->id) }}" method="POST">
                                            @csrf
                                            @method('PUT')
...
<div class="modal fade" id="addModal" tabindex="-1" aria-labelledby="addModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="addModalLabel">Add New Riwayat Jabatan Struktural</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form action="{{ route('riwayat-jabatan-struktural.store') }}" method="POST">
                        @csrf
                        ...
                        <div class="mb-3">
                            ...
                            <select name="jabatan_struktural_id" id="jabatan_struktural_id" class="form-select" required>
                                @foreach($jabatanStrukturals as $jabatanStruktural)
                                    <option value="{{ $jabatanStruktural->id }}">{{ $jabatanStruktural->name }}</option>
                                @endforeach
                            </select>
                        </div>
