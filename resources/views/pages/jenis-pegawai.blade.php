...
<div class="pagetitle">
    ...
    <nav>
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active">Jenis Pegawai</li>
      </ol>
    </nav>
  </div><!-- End Page Title -->
  ...
  <div class="card-body">
      ...
      <h6 class="card-subtitle mb-2 text-body-secondary">Jenis Pegawai</h6>
      ...
      <tbody>
          ...
          <tr>
              ...
              <td>
                  ...
                  <form action="{{ route('jenis-pegawai.destroy', $jenisPegawai->id) }}" method="POST" style="display:inline;" id="delete-form-{{ $jenisPegawai->id }}">
                      @csrf
                      @method('DELETE')
                      <button type="button" class="btn btn-sm btn-danger" onclick="confirmDelete({{ $jenisPegawai->id }})">Delete</button>
                  </form>
              </td>
...
<div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h1 class="modal-title fs-5" id="exampleModalLabel">Tambah Jenis Pegawai</h1>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <!-- Form to insert data into JenisPegawaiController -->
        <form id="jenisPegawaiForm" action="{{ route('jenis-pegawai.store') }}" method="POST">
          @csrf <!-- Include CSRF token for security -->
          <div class="mb-3">
            <label for="jenisPegawaiName" class="form-label">Nama Jenis Pegawai</label>
            <input type="text" class="form-control" id="jenisPegawaiName" name="name" required>
          </div>
        </form>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
        <button type="submit" class="btn btn-primary" form="jenisPegawaiForm">Submit</button>
      </div>
    </div>
  </div>
</div>
